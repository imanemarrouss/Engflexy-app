package ma.zsmart.engflexy.zynerator.converter;


import ma.zsmart.engflexy.zynerator.audit.AuditBusinessObject;
import ma.zsmart.engflexy.zynerator.bean.BusinessObject;
import ma.zsmart.engflexy.zynerator.bean.Etablissement;
import ma.zsmart.engflexy.zynerator.dto.AuditBaseDto;
import ma.zsmart.engflexy.zynerator.dto.BaseDto;
import ma.zsmart.engflexy.zynerator.dto.EtablissementDto;
import ma.zsmart.engflexy.zynerator.exception.CustomException;
import ma.zsmart.engflexy.zynerator.util.*;
...
public abstract class AbstractConverter<T extends BusinessObject, DTO extends BaseDto, H extends BusinessObject> {
    ...
    public T getById(DTO dto) throws CustomException {
        T result = null;
        if (dto != null) {
            try {
                ...
                throw new CustomException("Error while getting object by ID: " + e.getMessage());
            }
        }
        return result;
    }
    ...
    public DTO getById(T t) throws CustomException {
        DTO result = null;
        if (t != null) {
            try {
                result = dtoType.getDeclaredConstructor(Long.class).newInstance(t.getId());
                     ...
                     InvocationTargetException e) {
                throw new CustomException("Error while getting DTO by ID: " + e.getMessage());
            }
        }
        return result;
    }
