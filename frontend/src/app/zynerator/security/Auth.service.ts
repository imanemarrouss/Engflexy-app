...
export class AuthService {
    ...
    private loginResultSubject: Subject<boolean> = new Subject<boolean>();
    public loginResult$: Observable<boolean> = this.loginResultSubject.asObservable();

   public loginAdmin(username: string, password: string): void {
        // Check if remaining attempts are greater than 0
        if (this.remainingAttempts > 0) {
            this.http.post<any>(this.API + 'login', { username, password }, { observe: 'response' }).subscribe(
                resp => {
                    ...
                    console.log('You are logged in successfully');
                    this.router.navigate(['/' + environment.rootAppUrl + '/admin']);
                    this.loginResultSubject.next(true); // Emit login success
                }, (error: HttpErrorResponse) => {
                    this.error = error.error;
                    console.log(error);
                    this.remainingAttempts--; // Decrement the remaining attempts
                    if (this.remainingAttempts === 0) {
                        console.log('You are blocked for 30 minutes');
                        // Display the blocked message or handle the blocking logic
                    } else {
                        console.log('Wrong password. Remaining attempts:', this.remainingAttempts);
                        // Display the remaining attempts message
                    }
                    this.loginResultSubject.next(false); // Emit login failure
                }
            );
        } else {
            console.log('You are blocked for 30 minutes');
